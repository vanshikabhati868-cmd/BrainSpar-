...
export default function BrainSpar() {
  ...
  return (
    <>
      ...
      `}</style>
      <div style={{
        minHeight: "100vh", background: "#0d0f18", color: "#e8e8f0",
        ...
        {stage === "input" && (
          <div style={{ width: "100%", maxWidth: 660, marginTop: 36 }}>
            <div style={{ fontSize: 28, fontWeight: 700, marginBottom: 8, lineHeight: 1.2 }}>
              Paste your notes.<br />
              <span style={{ color: "#f0c040" }}>We'll debate until you get it.</span>
            </div>
            <div style={{ color: "#6b6e8a", fontSize: 14, marginBottom: 28 }}>
              BrainSpar challenges your understanding so the material actually sticks.
            </div>
                ...
                <div style={{ padding: 24, minHeight: 260, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 14 }}>
                  ...
                  {!pdfName ? (
                    <>
                      ...
                      <div style={{ color: "#6b6e8a", fontSize: 14, textAlign: "center" }}>Upload a PDF of your notes or textbook</div>
                      <button onClick={() => fileRef.current?.click()} style={{ background: "#f0c04018", border: "1px solid #f0c040", color: "#f0c040", borderRadius: 8, padding: "10px 24px", cursor: "pointer", fontFamily: "Georgia, serif", fontSize: 14, fontWeight: 600 }}>Choose PDF</button>
                    </>
                  ) : (
                    <>
                      <div style={{ fontSize: 44 }}>✅</div>
                      <div style={{ color: "#4ade80", fontWeight: 600, fontSize: 15 }}>{pdfName}</div>
                      <button onClick={() => fileRef.current?.click()} style={{ background: "transparent", border: "1px solid #2a2d3d", color: "#6b6e8a", borderRadius: 8, padding: "6px 16px", cursor: "pointer", fontSize: 13 }}>Change file</button>
                    </>
                  )}
                </div>
              )}
            </div>
            <button onClick={startDebate} disabled={!hasContent} style={{ width: "100%", marginTop: 14, padding: "16px 0", background: hasContent ? "#f0c040" : "#2a2d3d", color: hasContent ? "#111" : "#6b6e8a", border: "none", borderRadius: 12, fontSize: 16, fontWeight: 700, fontFamily: "Georgia, serif", cursor: hasContent ? "pointer" : "not-allowed", transition: "all 0.2s" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14, minHeight: 380, maxHeight: "58vh", overflowY: "auto", paddingBottom: 8 }}>
              {messages.map((m, i) => (
                <div key={i} style={{ display: "flex", flexDirection: m.role === "user" ? "row-reverse" : "row", alignItems: "flex-start", gap: 10 }}>
                  <div style={{ width: 34, height: 34, borderRadius: "50%", flexShrink: 0, background: m.role === "assistant" ? "#f0c040" : "#1e2235", border: m.role === "user" ? "1px solid #f0c04066" : "none", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>
                    {m.role === "assistant" ? "🎓" : "🧑"}
                  </div>
                  <div style={{ maxWidth: "78%", background: m.role === "assistant" ? "#1a1d27" : "#1e1a0d", border: `1px solid ${m.role === "assistant" ? "#2a2d3d" : "#3a3010"}`, borderRadius: m.role === "assistant" ? "4px 14px 14px 14px" : "14px 4px 14px 14px", padding: "12px 16px", fontSize: 14, lineHeight: 1.7, color: "#e8e8f0" }}>
                    {m.content}
                  </div>
                </div>
              ))}
              {loading && (
                <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                  <div style={{ width: 34, height: 34, borderRadius: "50%", background: "#f0c040", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>🎓</div>
                  <div style={{ background: "#1a1d27", border: "1px solid #2a2d3d", borderRadius: "4px 14px 14px 14px", padding: "12px 18px", fontSize: 20, color: "#f0c040" }}>···</div>
                </div>
              )}
              <div ref={chatEndRef} />
            </div>
